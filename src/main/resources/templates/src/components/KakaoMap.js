        ...
        <Map center={location} style={{ width: '50%', height: '400px' }} level={3}>
            {
                list.map((v, i) => <CustomOverlayMap position={v.location} key={i}>
                    <CustomMarker link={v.link} title={v.title} img={v.img} />
                </CustomOverlayMap>)
            }
        </Map>
    );
